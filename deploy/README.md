...
官方 `v0.27.1` amd64 digest 为 `sha256:c2f3b1b964e47809b722b5e75b61b1e7b39a50f70388cf2bf2418f16a9f31da2`。正式环境必须在拉取后校验并将 `VLLM_ALLOWED_IMAGES` 改为完整 `repository@sha256:...` 引用，评测镜像也必须推送内部仓库并把 `EVALUATION_ALLOWED_IMAGES` 改成 digest；预检会拒绝生产配置中的可变 tag。默认镜像基于 CUDA 13.0.2；若 AutoDL/内网服务器的 NVIDIA 驱动不满足要求，可改用官方 `v0.27.1-cu129` amd64 digest `sha256:6666717cd1cadf9adfff8abec9c3f2eca6e27e742de06fe7d7f129fa3d647732`，同时用它重建评测镜像；不要回退到旧 vLLM 版本。开发机可显式设置 `ENVIRONMENT=development` 使用固定 tag。
