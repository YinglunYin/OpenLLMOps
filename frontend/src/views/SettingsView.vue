...
const sourceForm = reactive({ hfToken: useMocks ? 'hf_••••••••••••••••••••' : '', msToken: useMocks ? 'ms_••••••••••••••••••••' : '', inbox: useMocks ? '/data/openllmops/models/inbox' : '', safetensorsOnly: true, trustRemoteCode: false, pinRevision: true })
...
<template>
  <div>
    <PageHeader title="系统设置" subtitle="配置在线模型来源、受控存储、安全策略与运行时版本" />
    <el-alert v-if="!useMocks" class="settings-api-alert" title="部分设置端点尚未接入" description="API Key 已连接真实控制面；在线模型源、存储、HTTPS 与运行时版本只呈现配置状态和明确空态。" type="warning" :closable="false" show-icon />
      ...
      <el-tab-pane label="模型来源" name="sources">
        <div class="settings-layout">
          <div class="settings-main">
            ...
            <PanelCard title="人工拷贝与导入" class="section-gap"><div class="inbox-row"><span>受控目录（只读）</span><el-input v-model="sourceForm.inbox" readonly/><StatusPill :text="useMocks ? '目录可用' : '未读取'" :tone="useMocks ? 'success' : 'info'"/></div><el-alert title="平台不连接 SFTP，也不保存服务器凭证" description="管理员自行通过 SSH/SFTP 登录服务器，将模型复制到受控目录；完成后在“模型资产”页面扫描并人工确认导入。" type="info" :closable="false" show-icon/></PanelCard>
